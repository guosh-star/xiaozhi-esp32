  ...
  try{
    ...
    def _handle_stream_inner(self, params, q):
        ...
        if songs and len(songs) >= 2:
            ...
            for s in songs[:5]:
                ...
                sn_clean = _re.sub(r'[\(（][^)）]*[\)）]', '', sn).strip()
                top_clean = _re.sub(r'[\(（][^)）]*[\)）]', '', top_name).strip()
            ...
            if len(artists) >= 2:
                ...
                for key, _ in st.get("qmap", []):
                    if _re.search(_normalize(key), _normalize(q)):
                        is_aliased = True
                        break
                if not is_aliased:
    ...
    def _stream_ffmpeg_inner(self, keyword, fmt="pcm"):
            ...
            try:
                ...
                if songs and len(songs) >= 2:
                    ...
                    if len(artists) >= 2:
                        ...
                        sn_only = _re.sub(r'[\(（][^)）]*[\)）]', '', _normalize(song_name)).strip()
        ...
        try:
            ...
            # 先立即返回 200 + 响应头，避免 ESP32 端 HTTP 等待超时
            if fmt == "pcm":
                ct = "audio/raw"
            else:  # opus
            ...
            self.send_header("X-Sample-Rate", "32000")
            self.send_header("X-Song-Name",
                song_info["name"].encode("ascii", errors="replace").decode("ascii"))
            self.send_header("X-Song-Singer",
                song_info["singer"].encode("ascii", errors="replace").decode("ascii"))
            ...
            # ffmpeg 直连在线源 + 服务器端看门狗自动续接。
            # 网易云/QQ 签名 URL 有时效（实测 4-5 分钟掐断），服务器端检测断流后
            # 自动重新签名 URL 并续播，ESP32 侧完全无感知。最多重试 3 次。
            ...
            def _build_ff_args(inp):
                if fmt == "pcm":
                    return [ffmpeg_path, "-nostdin", "-loglevel", "error"] + net_opts + [
                        "-i", inp,
                        "-af", "aresample=32000:resampler=swr:osf=s16:dither_method=none",
                        "-c:a", "pcm_s16le",
                        "-f", "s16le",
                        "-ar", "32000",
                        "-ac", "1",
            ...
            proc = subprocess.Popen(_build_ff_args(input_src), stdout=subprocess.PIPE,
                                    stderr=subprocess.DEVNULL)

            # 限速发送：前 128KB 双倍速，之后正常速（64KB/s）
            total = 0
            warmup = True
            ...
            ffd = proc.stdout.fileno()
            while True:
                try:
                    r, _, _ = _sel.select([ffd], [], [], 8.0)
                ...
                if r:
                    try:
                        ...
                        chunk = None
                if not chunk:
                    # 区分：歌正常播完 vs 源断流 crash vs select 超时（进程还活着）
                    rc = proc.poll()
                    if rc is None:
                        # ffmpeg 还在运行，只是暂时没数据 → 继续等
                        continue
                    if rc == 0:
                        # 正常结束（returncode=0），不重启
                        print(f"[{fmt}] ffmpeg exited normally (rc=0)")
                        break
                    # ffmpeg 异常退出 → 并行：杀旧进程 + 获取新 URL
                    import threading as _th
                    new_url_box = [None]
                    def _fetch_new_url():
                        for __ in range(10):
                            u, _ = get_play_url_for(song_info)
                            if u:
                                new_url_box[0] = u
                                return
                            time.sleep(2)
                    _fetch_t = _th.Thread(target=_fetch_new_url, daemon=True)
                    _fetch_t.start()
                    try:
                        proc.stdout.close()
                    except:
                    ...
                    try:
                        ...
                        proc.kill()
                    if client_gone or song_info.get("source") == "local" or restart_count >= 3:
                        break
                    restart_count += 1
                    elapsed = time.monotonic() - stream_start
                    print(f"[{fmt}] Watchdog #{restart_count}: stalled at {elapsed:.0f}s rc={rc}, re-signing...")
                    _fetch_t.join(timeout=15)
                    new_url = new_url_box[0]
                    if not new_url:
                        print(f"[{fmt}] Cannot re-sign URL after {restart_count} tries")
                        break
                    seek_args = _build_ff_args(new_url)
                    # 不用 -ss（MP3 关键帧 seek 不精确），改从歌头播放，
                    # 服务器端精确跳过已发送的 PCM 字节实现字节级对齐。
                    # PCM: 16000 Hz × 2 bytes/sample = 32000 bytes/second
                    skip_bytes = int(elapsed * 32000)
                    proc = subprocess.Popen(seek_args, stdout=subprocess.PIPE,
                                            stderr=subprocess.DEVNULL)
                    ffd = proc.stdout.fileno()
                    skipped = 0
                    while skipped < skip_bytes:
                        chunk = os.read(ffd, min(65536, skip_bytes - skipped))
                        if not chunk:
                            break
                        skipped += len(chunk)
                    print(f"[{fmt}] Skipped {skipped}/{skip_bytes} bytes, resuming at {elapsed:.0f}s")
                    continue
                try:
            ...
            if client_gone:
                ...
                print(f"[{fmt}] Done: {total} bytes, restarts={restart_count}")
