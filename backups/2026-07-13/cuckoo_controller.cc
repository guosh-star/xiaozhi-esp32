...
int Mp3Player::DecodeSingleFile(int index) {
    ...
    // 跳过 ID3v2 标签（MP3 文件以 "ID3" 开头时，前 10 字节后的 sync-safe int 是标签长度）
    ...
    if (mp3_size > 10 && memcmp(mp3_start, "ID3", 3) == 0) {
        ...
        if (skip < mp3_size - 1024) {  // 确保跳过后还有足够数据
            mp3_start += skip;
            mp3_data_size -= skip;
    ...
    while (remaining > 0 && !stop_requested_) {
        ...
        if (ret == ESP_AUDIO_ERR_OK) {
            // 首次获取解码信息 — esp_mp3_dec_decode 已通过 dec_info 参数填充
 ...
 // ---- Smooth ducking: fade music out when AI starts speaking ----
                ...
                if (duck_state == 1) {
                    ...
                    if (!ai_speaking) {
                        // AI stopped during fade — restore
                        duck_state = 3;
                        duck_transition_us = esp_timer_get_time();
                    }
...
CuckooStateMachine::CuckooStateMachine(Motor* m1, Motor* m2, Motor* m3, Motor* m4,
    ...
    : m1_(m1), m2_(m2), m3_(m3), m4_(m4),
      ...
      water_bird_(water_bird), mp3_(mp3), bell_player_(bell_player), ldr_(ldr),
...
// ============================================
// 共用舞蹈序列（PerformanceTask + ShowTask 调用）
// ============================================
...
void CuckooStateMachine::PlayDogBark() {
    // 从 assets 读取 dog_bark.wav (16kHz mono s16)
    ...
    // 把狗叫 PCM 加载到 Mp3Player，DecodeSingleFile 的输出循环会自动混音
...
void CuckooStateMachine::RunDanceFinale() {
    // 音乐结束，两串灯珠一齐亮（直到大门关闭后在 PerformanceTask/ShowTask 末尾熄灭）
...
void CuckooStateMachine::PerformanceTask(void* arg) {
 ...
 // 静音恢复 AI
    ...
    sm->MotorPowerOff();
    // 恢复唤醒词检测（不调 EnableVoiceProcessing — 避免 ResetDecoder 干扰 TTS）
    // 只在 idle 状态才恢复，避免在 listening 状态误触发唤醒词
    if (app.GetDeviceState() == kDeviceStateIdle) {
        app.GetAudioService().EnableWakeWordDetection(true);
    }
...
void CuckooStateMachine::StartShow() {
 // 整点/半点报时任务正在运行 不允许 Show 打断AI 误调保护
    ...
    // 表演期间禁掉语音处理和唤醒词检测，防止音乐被误认唤醒词
    // 导致音频通路冲突 → esp_codec_dev_write panic
    app.GetAudioService().EnableVoiceProcessing(false);
    app.GetAudioService().EnableWakeWordDetection(false);
...
void CuckooStateMachine::ShowTask(void* arg) {
    ...
    // 等 TTS-STOP 处理完毕（AbortSpeaking 触发的 TTS-STOP 可能延迟3秒
    // 将状态 speaking→listening 并重新启用 voice processing，覆盖 StartShow 里的禁用）
    vTaskDelay(pdMS_TO_TICKS(500));
    auto& app = Application::GetInstance();
    app.GetAudioService().EnableVoiceProcessing(false);
    app.GetAudioService().EnableWakeWordDetection(false);
    ...
    // 舞蹈+小提琴立刻开始（和开门/小狗出场并行）
    sm->RunDanceLoop();

    // 音乐结束，恢复唤醒词和语音处理
    // 注意：StartShow 的 AbortSpeaking 可能让状态卡在 listening（非 idle），
    // 直接恢复语音处理会导致唤醒词在 listening 状态触发 AbortSpeaking → audio_input 死锁
    sm->is_running_ = false;
    sm->current_performance_ = kPerformanceNone;
    app.GetAudioService().SetOutputMuted(false);
...
int CuckooStateMachine::PlayOnlineMusic(const char* url_or_path) {
 ...
 // URL-encode non-ASCII chars (Chinese etc.), esp_http_client does not support raw Chinese URLs
    ...
    while (*src && dst < end) {
        ...
        if (c == ' ') {
            // 空格在 HTTP request line 中是分隔符，必须编码
            if (dst + 3 <= end) {
                *dst++ = '%';
                *dst++ = '2';
                *dst++ = '0';
...
void cuckoo_clock_task(void* params) {
 ...
 // AI说话时小鸟跟随话音节奏跳动（不在表演中且无背景音乐时）
        ...
        // NTP 同步：未同步时每 60 秒重试，同步后每 5 分钟校准一次
        // 频繁校准消除软件时钟累积漂移导致的半点/整点报时不准
 ...
 // 音频电源管理会在输入/输出空闲 15 秒后关闭 I2S导致唤醒/喇叭失效
        ...
        // 如果时间已通过 NTP 或 MCP 设置，用内部时间
        if (sm->time_set_) {
            // NTP 已同步时直接用系统时间，避免软件时钟漂移导致报时不准
            struct timeval tv;
            gettimeofday(&tv, nullptr);
            if (tv.tv_sec > 1000000000) {
                struct tm timeinfo;
