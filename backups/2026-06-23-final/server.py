...
def unified_search(keyword, limit=10):
    """Search Netease first, fill with QQ Music (dedup)"""
    all_songs = []

    songs, _ = netease_search(keyword, limit)
    if songs:
        all_songs.extend(songs)

    songs2, _ = qq_search(keyword, min(5, limit))
    if songs2:
    ...
    if not all_songs:
        return None, "No results from either platform"
...
def get_play_url_for(song):
    ...
    if source == "netease":
        return netease_play_url(int(sid))
    else:
        return qq_play_url(song.get("mid", sid))
...
async function play(el){
  ...
  try{
    ...
    def _handle_quick(self, params):
        ...
        # Try each song for a playable URL
        for song in songs[:3]:
    ...
    def _handle_stream(self, params):
        ...
        songs_for_cache, _ = unified_search(q, 1)
        if songs_for_cache:
            song_id = str(songs_for_cache[0].get("id", q))
            cached = _cache_get(song_id)
            if cached:
                print(f"[stream] CACHE HIT: {songs_for_cache[0]['name']} - {songs_for_cache[0]['singer']}")
                self._send_file(cached, songs_for_cache[0])
                return

        # === v4: Send headers IMMEDIATELY with chunked encoding ===
        # This keeps the ESP32 HTTP client alive while we search + download.
        self.send_response(200)
        self.send_header("Content-Type", "audio/mpeg")
        self.send_header("Transfer-Encoding", "chunked")
        self.end_headers()

        # Now search and get playable URL (ESP is waiting but not timing out)
        try:
            songs, err = unified_search(q, 5)
            if err or not songs:
                self._send_chunk(b'')  # end chunked response
                return
            ...
            with urllib.request.urlopen(req, timeout=120, context=ctx) as upstream:
                total_sent = 0
                while True:
                    chunk = upstream.read(262144)  # 256KB reads
                    if not chunk:
                        break
            ...
            _cache_put(song_id, bytes(cache_buf))
    ...
    # -----------------------------------------------------------------------
    # /pcm - Raw PCM streaming via ffmpeg
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # /opus - Opus OGG streaming via ffmpeg
    # -----------------------------------------------------------------------
    ...
    def _find_playable(self, keyword):
        ...
        songs, err = unified_search(keyword, 5)
        if err or not songs:
            return None, None
        for song in songs[:5]:
            url, e = get_play_url_for(song)
    ...
    def _stream_ffmpeg(self, keyword, fmt="pcm"):
        ...
        Tries multiple search results if the first source is too short
        (e.g. 20s preview instead of full song)."""
        import subprocess

        print(f"[{fmt}] Searching: {keyword}")
        songs, err = unified_search(keyword, 10)
        if err or not songs:
            self.send_error(404, f"No result for: {keyword}")
            return

        # Minimum bytes for a reasonable song (~1 min at 16kHz mono s16le)
        MIN_BYTES = 2 * 1024 * 1024  # 2 MB

        try:
            ffmpeg_path = r"C:\Users\GUO-notebook\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.1.1-full_build\bin\ffmpeg.exe"
            ...
            # Send headers once for the whole multi-source stream (chunked)
            if fmt == "pcm":
                ct = "audio/raw"
            else:
            ...
            for song in songs:
                audio_url, e = get_play_url_for(song)
                if not audio_url:
                    print(f"[{fmt}] Skip {song['name']} - {song['singer']}: no URL")
                    continue

                print(f"[{fmt}] Trying: {song['name']} - {song['singer']}")
                ...
                if fmt == "pcm":
                    args = [
                        ...
                        "-i", audio_url,
                        "-af", "aresample=16000:resampler=swr:osf=s16:dither_method=none",
                        "-c:a", "pcm_s16le",
                        "-f", "s16le",
                        "-ar", "16000",
                        "-ac", "1",
                    ...
                    while True:
                        ...
                        try:
                            for i in range(0, len(chunk), SUB_CHUNK):
                                ...
                                self.wfile.flush()
                            song_total += len(chunk)
                            if warmup and (song_total + grand_total) >= 131072:
                                warmup = False
                        except OSError:
                            break
                except OSError:
                ...
                grand_total += song_total
                print(f"[{fmt}] {song['name']} done: {song_total} bytes (total: {grand_total})")

                if song_total >= MIN_BYTES:
                    print(f"[{fmt}] Good source, stopping search")
                    break
                else:
                    print(f"[{fmt}] Too short ({song_total} < {MIN_BYTES}), trying next...")

            self._send_chunk(b'')
            print(f"[{fmt}] All done: {grand_total} bytes")
...
def main():
    ...
    print("=" * 50)
    print("  Music Proxy Service v3.0 (NetEase + QQ Music)")
