...
RTC_NOINIT_ATTR struct {
    uint32_t magic;        // 魔数 0xCAFEBABE 表示数据有效
    uint32_t tick_sec;     // 最后心跳的秒数
    uint8_t  dev_state;    // 最后设备状态
    uint8_t  music_active; // 音乐是否在播
    ...
    while (1) {
        ...
        if (tick_sec % 86400 == 0) {
            ...
            if (tv.tv_sec > 1000000000) {
                ...
                sm->current_min_ = timeinfo.tm_min;
                sm->current_sec_ = timeinfo.tm_sec;
                sm->time_set_ = true;  // 确保时钟启动（即使开机初始同步失败）
                ESP_LOGI(TAG, "Daily NTP sync: %02d:%02d:%02d",
                         sm->current_hour_.load(), sm->current_min_.load(), sm->current_sec_.load());
            }
        }

        // 音乐/背景音频播放中，禁止 WiFi 省电（框架 state transition
