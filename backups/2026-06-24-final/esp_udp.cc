...
bool EspUdp::Connect(const std::string& host, int port) {
    ...
    xTaskCreate([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "udp_receive", 3072, this, 5, &receive_task_handle_);
    return true;
}
...
void EspUdp::ReceiveTask() {
    std::string data;
    int64_t last_recv_ok = esp_timer_get_time();
    while (connected_) {
        data.resize(1500);
        ...
        // Diagnostic: warn if recv() was blocked for >500ms
        // (indicates lwIP/TCP starvation, not WiFi signal loss)
        int64_t now = esp_timer_get_time();
        if (now - last_recv_ok > 500000) {
            ESP_LOGW(TAG, "UDP recv gap: %d ms", (int)((now - last_recv_ok) / 1000));
        }
        last_recv_ok = now;
