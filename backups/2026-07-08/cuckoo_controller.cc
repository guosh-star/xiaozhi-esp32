...
// PWM ģʽ — С���� (timer0, 1kHz/10-bit, ƥ�� dog-test)
...
// BirdJump �����Ƴ���ˮ��+�������� Motor �� (DRV8833 IN1/IN2 ��������)
// ��� config.h: MOTOR_WATER_BIRD_IN1/IN2 + LEDC_CH_WATER/JUMP
// ˮ��: water_bird_->SetSpeed(100) �� IN1=HIGH
...
// PlayUrl - HTTP������Ƶ������
// ֧��:
//   1. ֱ�� MP3 URL (Content-Type: audio/*)
//   2. �������� /stream �˵� (ֱ�ӷ�����Ƶ)
//   3. �������� JSON ��Ӧ (��ȡ url �ֶκ�����)
// ============================================
...
// �ϸ� MPEG ֡ͷУ�飺������� FF Ex����У�� bitrate/samplerate/layer
    ...
    while (1) {
        while (rem > 0 && !self->stop_requested_) {
            ...
            if (dec_ret == ESP_AUDIO_ERR_OK) {
                ...
                // �ý��������ص� frame_size ���� raw.consumed ȷ��֡�߽羫ȷ
        ...
        ptr = buf;
        rem = carry + batch_len;
        // carry=0�ҷ�֡ͷ��ɨ������metadata/��֡���ݵ���һ���ϸ�֡ͷ
...
// ���� PlayPcm: HTTP����raw PCM �� PushRawPcmToPlayback����ԭ�����Ź��ߣ���֡�������⣩����
...
serial_fallback:
    ...
    if (use_opus) {
        ...
        while (self->is_playing_ && !self->stop_requested_) {
            ...
            // for a 3�C5 min song during a 5�C15 s AI response).
            auto dev_state = app.GetDeviceState();
...
// ============================================
// BellSoundPlayer - ͨ�� AI ��Ƶϵͳ���Ų������??// ============================================
void BellSoundPlayer::PlayCuckooSoundSync() {
    // ֱ��д I2S������ֱ�����꣩��ʹ�� data_if_mutex_ �� AudioOutputTask ����
...
CuckooStateMachine::CuckooStateMachine(Motor* m1, Motor* m2, Motor* m3, Motor* m4,
    ...
    : m1_(m1), m2_(m2), m3_(m3), m4_(m4),
      bird_door_(bird_door), violin_servo_(violin), dog_servo_(dog),  // bird_door_ 实际是【小提琴电机】
    ...
    // �����ԴP-MOSFET���� (GPIO LOW=ON, HIGH=OFF)
...
void CuckooStateMachine::StartPerformance(PerformanceType type, int hour) {
    ...
    // ��AI�󴥷����豸�մ�idle����5���ڣ����� performance��AI�����ڻ���ʱ�Զ���cuckoo.performance��
...
void CuckooStateMachine::PerformanceTask(void* arg) {
    ...
    if (sm->current_performance_ == kPerformanceHour) {
        ...
        // Phase 2: bell chime - ����0013.mp3��PCM��������ѭ��������MP3����������
        ...
        if (sm->water_bird_) sm->water_bird_->SetSpeed(100);  // ˮ��
        ...
        if (sm->mp3_ && sm->total_calls_ >= 1 && sm->total_calls_ <= 12) {
            ...
            int v_fwd2 = 0, v_rev2 = 0;  // violin fwd/rev counter
            ...
            while (sm->is_running_ && sm->mp3_->IsPlaying()) {
                ...
                // 小提琴循环动作：每帧 9° 平滑扫动
                static int v_stage2 = 0;
                static int v_loop2 = 0;
                static int v_timer2 = 0;
                static int v_angle2 = 90;
                v_timer2 += 50;
                switch (v_stage2) {
                    case 0: if (v_angle2 < 90) v_angle2+=9; else if (v_angle2 > 90) v_angle2-=9; break;
                    case 1: if (v_angle2 > 0) v_angle2-=9; break;
                    case 2: if (v_angle2 < 180) v_angle2+=9; break;
                    case 3: if (sm->bird_door_) sm->bird_door_->Forward(100); break;
                    case 4: if (v_angle2 > 90) v_angle2-=9; else if (v_angle2 < 90) v_angle2+=9; break;
                    case 5: if (v_angle2 < 180) v_angle2+=9; break;
                    case 6: if (v_angle2 > 0) v_angle2-=9; break;
                    case 7: if (sm->bird_door_) sm->bird_door_->Reverse(100); break;
                    case 8: if (v_angle2 < 130) v_angle2+=9; else if (v_angle2 > 130) v_angle2-=9; break;
                    case 9: if (v_angle2 > 90) v_angle2-=9; else if (v_angle2 < 90) v_angle2+=9; break;
                }
                if (v_angle2 < 0) v_angle2 = 0;
                if (v_angle2 > 180) v_angle2 = 180;
                if (sm->violin_servo_) sm->violin_servo_->SetAngle(v_angle2);
                int d2[] = {500,800,1000,1000,600,1000,1200,1000,1500,300};
                if (v_timer2 >= d2[v_stage2]) {
                    if (v_stage2 == 3) { sm->bird_door_->Stop(); v_fwd2++; }
                    else if (v_stage2 == 7) { sm->bird_door_->Stop(); v_rev2++; }
                    v_stage2++;
                    if (v_stage2 >= 10) { v_stage2 = 0; v_loop2++; if (v_loop2 >= 3) v_loop2 = 0; }
                    v_timer2 = 0;
                }

                // С������ҡ�ڣ��������棩
                if (sm->dog_servo_) {
                    static int dog_angle = 0;
                    static int dog_dir = 1;
                    static int dog_target = 50;
                    static int dog_pause = 0;
                    if (dog_pause > 0) {
                        dog_pause--;
                    } else {
                        dog_angle += dog_dir;
                        if (dog_angle >= dog_target) {
                            dog_dir = -1;
                            dog_pause = 20 + (esp_random() % 40);
                        }
                        if (dog_angle <= 0) {
                            dog_dir = 1;
                            dog_target = 40 + (esp_random() % 21);
                            dog_pause = 0;
                        }
                        sm->dog_servo_->SetAngle(dog_angle);
                    }
                }
            ...
            // 音乐结束 → 平衡正反转次数
            if (sm->m1_) sm->m1_->Stop();
            if (v_rev2 < v_fwd2 && sm->bird_door_) {
                sm->bird_door_->Reverse(100);
                vTaskDelay(pdMS_TO_TICKS(1200));
            } else if (v_fwd2 < v_rev2 && sm->bird_door_) {
                sm->bird_door_->Forward(100);
                vTaskDelay(pdMS_TO_TICKS(1200));
            }
            if (sm->bird_door_) sm->bird_door_->Stop();
            if (sm->violin_servo_) sm->violin_servo_->SetAngle(90);
...
void CuckooStateMachine::CheckTime(int hour, int min, bool dark) {
    ...
    // �ɵ��÷����������㱨ʱ���ǰ�㱨ʱ
    // ʹ�� last_hour_ �� last_half_hour_ ���ظ����� cuckoo_clock_task ������
...
void CuckooStateMachine::Dance() {
    ...
    while ((xTaskGetTickCount() * portTICK_PERIOD_MS) < end) {
        ...
        // С���ٵ��: ������ת (board B M2 via bird_door_)
        if (bird_door_) bird_door_->Forward(60);
...
void CuckooStateMachine::StartShow() {
    ...
    // �ö�����������ִ�У�������MCP�����߳�
    // �󶨵� Core 1�����ȼ� 5 = mp3_play ͬ����FreeRTOS ��ת���ȸ��ܸ���
    xTaskCreatePinnedToCore(
        ShowTask,
        "cuckoo_show",
        4096,
...
void CuckooStateMachine::ShowTask(void* arg) {
    ...
    gpio_set_level(LED_A_GPIO, 1);
    gpio_set_level(LED_B_GPIO, 1);
    if (sm->water_bird_) sm->water_bird_->SetSpeed(100);  // ˮ�� IN1=HIGH
    ...
    while (sm->is_running_ && sm->mp3_->IsPlaying()) {
        // ��ϣ�ÿ�ֶ�����ˮ���ٶ�
        if (sm->water_bird_) sm->water_bird_->SetSpeed(100);
        unsigned long now = xTaskGetTickCount() * portTICK_PERIOD_MS;
        ...
        // 小提琴循环动作：每帧 9° 平滑扫动
        static int v_stage = 0;
        static int v_loop = 0;
        static int v_timer = 0;
        static int v_angle = 90;
        v_timer += 50;
        switch (v_stage) {
        ...
        if (v_timer >= st_dur[v_stage]) {
            if (v_stage == 3) { sm->bird_door_->Stop(); v_fwd++; }
            else if (v_stage == 7) { sm->bird_door_->Stop(); v_rev++; }
            v_stage++;
            if (v_stage >= 10) { v_stage = 0; v_loop++; if (v_loop >= 3) v_loop = 0; }
            v_timer = 0;
        }

        // С������ҡ�ڣ���������ÿ 50ms �� 1�㣬���� ShowTask��
        if (sm->dog_servo_) {
            static int dog_angle = 0;
            static int dog_dir = 1;
            static int dog_target = 50;
            static int dog_pause = 0;
            if (dog_pause > 0) {
                dog_pause--;  // ÿ��һ��ѭ���� 1��50ms/��������ÿ 50ms ����һ��
            } else {
                dog_angle += dog_dir;
                if (dog_angle >= dog_target) {
                    dog_dir = -1;
                    dog_pause = 20 + (esp_random() % 40);  // 1~3 ����ͣ
                }
                if (dog_angle <= 0) {
                    dog_dir = 1;
                    dog_target = 40 + (esp_random() % 21);
    ...
    // ====== 音乐结束 → 平衡正反转次数 ======
    if (sm->m1_) sm->m1_->Stop();
    if (v_rev < v_fwd && sm->bird_door_) {
        ESP_LOGI(TAG, "Violin: fwd=%d rev=%d, adding reverse", v_fwd, v_rev);
        sm->bird_door_->Reverse(100);
        vTaskDelay(pdMS_TO_TICKS(1000));
    } else if (v_fwd < v_rev && sm->bird_door_) {
...
void CuckooStateMachine::StopAll() {
    ...
    if (m3_) m3_->Stop();  // С��
    if (m4_) m4_->Stop();  // ����
    if (bird_door_) bird_door_->Stop();  // С����
    if (violin_servo_) violin_servo_->SetAngle(90);
    ...
    // ͨ�� Schedule ����ѭ���а�ȫ�� idle + �ָ����Ѽ��
    // ��ֱ�� AbortSpeaking �� TTS ���������������ţ�������Ȼ����
    auto state = Application::GetInstance().GetDeviceState();
    if (state == kDeviceStateSpeaking || state == kDeviceStateListening) {
...
void CuckooStateMachine::SetMotorSpeed(int motor_id, int speed) {
    ...
    switch (motor_id) {
        ...
        case 2: if (bird_door_) bird_door_->SetSpeed(speed); break;  // 小提琴电机 (bird_door_ 是历史遗留变量名)
...
int CuckooStateMachine::PlayOnlineMusic(const char* url_or_path) {
    ...
    // URL�����ASCII�ַ������ĵȣ���esp_http_client��֧��ԭʼ����URL
...
void cuckoo_clock_task(void* params) {
    ...
    // �ڲ���������������������ʵ RTC����ͨ�� cuckoo.set_time MCP ����У׼��

    // ����ʱ���Դ� ESP32 ϵͳʱ�䣨WiFi NTP���Զ�ͬ��
    ...
    while (1) {
        ...
        // ����/������Ƶ�����У���ֹ WiFi ʡ�磨��� state transition
        // �ᷴ���л� LOW_POWER���˵�ʡ��ģʽ�� TCP ���ؿ��� �� ���ַ�����
