...
void Mp3Player::PlayOpusTask(void* arg) {
 ...
 // connect+5볬ʱSO_SNDTIMEOlwipϲЧ
 // ֹgoto serial_fallbackʼ
    {
...
/**
 * @brief ringtone
 * @param volume ϵ 0.0~1.0忪ͷ15%ǿ100%
 * A5(880Hz)/C#6(1100Hz)PCMÿ2
 */
...
/**
 * @brief ѭ
 * FreeRTOSѯ pending_track_ / pending_bell_hour_
 * ʱAIŶӦƵָAI
 */
    // 
...
void Mp3Player::Stop() {
 ...
 // Ҫڴis_playing_=false  PlayOpusTaskԼ
 // PlayOpus()Կɿصȴ˳
    pending_track_ = 0;
 ...
 // ƵУþWaitForPlaybackQueueEmpty()ٷ
...
// ============================================
// BellSoundPlayer - ͨAIƵϵͳ
// ============================================
void BellSoundPlayer::PlayCuckooSoundSync() {
    // Ҫ SetOutputMutedҲҪ vTaskDelay
    auto& app = Application::GetInstance();
...
void BellSoundPlayer::PlayBellSoundSync() {
    // ֱд I2S ֱ꣬ʹ data_if_mutex_  AudioOutputTask 
    auto& app = Application::GetInstance();
    app.GetAudioService().OutputRawPcm(
        cuckoo_bell_sound,
...
CuckooStateMachine::CuckooStateMachine(Motor* m1, Motor* m2, Motor* m3, Motor* m4,
    ...
    : m1_(m1), m2_(m2), m3_(m3), m4_(m4),
      violin_motor_(violin_motor), violin_servo_(violin), dog_servo_(dog),  // Сٵ (B M2, GPIO18/45)
